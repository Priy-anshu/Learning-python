...
params = {
    "key" : "32b1fde5e159424ab71113530262601",
    "q" : city,
    "aqi" : "no"
}
...
try :
    ...
    if 'error' in wData :
        print("error, ",wData['error']['message'])
    else :
        location = wData['location']
        current = wData['current']

        print("\n\nlocation :-\n")
        print("time : " , location['localtime'])
        print("city : " , location['name'])
        print("state : " , location['region'])
        print("country : " , location['country'])
        print("time-zone : " , location['tz_id'])

        print("\nweather :-\n")
        print("temprature : ", current['temp_c'])
        print("mood : ", current['condition']['text'])
        print("wind(kph) : ", current['wind_kph'])
        print("humidity : ", current['humidity'])
        print("cloud : ", current['cloud'],"%")
        #to speak
        os.system(f"say -v Rishi 'it is {current['temp_c']}degree celcius in {location['name']}, {location['region']}, {location['country']}'")
except Exception as e:
    print(e)

'''
list of accents available in macOs 
to check type ' say -v "?" ' in cmd or terminal
